...
func TestJailHappyPath(t *testing.T) {
	...
	t.Run("long_running_jail", func(t *testing.T) {
		...
		// Create spec with long-running command
		runOutput(t, misbahBin, "jail", "create",
			"--spec", longSpecFile,
			"--name", longJailName,
		...
		// Test 6c: Stop jail gracefully
		output = runOutput(t, misbahBin, "jail", "stop", "--name", longJailName)
		require.Contains(t, output, "Stopping jail: "+longJailName)
		require.Contains(t, output, "stopped gracefully")
...
func TestJailWithResources(t *testing.T) {
	...
	// Manually add resources to spec
	data, err := os.ReadFile(specFile)
	require.NoError(t, err)

	// Add resources section
	modifiedSpec := string(data) + `
resources:
    memory: 512MB
    cpu_shares: 1024
    io_weight: 100
...
func TestJailErrorCases(t *testing.T) {
	...
	t.Run("invalid_jail_spec", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(invalidSpec, []byte("invalid: yaml: ["), 0644))
